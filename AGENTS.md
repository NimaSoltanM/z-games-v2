...
- **Backend**: Go — Fiber v3 + pgx with handwritten, parameterized SQL (PostgreSQL), in `backend/`
...
Every route's primary, loader-owned server data MUST follow the pattern in
`docs/tanstack/must-use.txt` exactly. Client-derived or global non-blocking data
whose query keys are not knowable in the route loader (for example an anonymous
`localStorage` cart) may use `useQuery`/`useQueries`, but must handle its own
loading and error states and must not replace loader prefetch for route data.
...
- Use `useQuery` instead of `useSuspenseQuery` for primary route data
